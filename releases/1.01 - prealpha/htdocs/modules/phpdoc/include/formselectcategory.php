 ...
 * @access 		public
 */
class XoopsFormSelectCategory extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectCategory($caption, $name, $value = null, $size = 1, $multiple = false, $ownid=0, $none = true)
    {
	...
	function GetCategories($ownid){
		...
		$categories = $category_handler->getObjects(new Criteria('parentid', 0), true);
	...
	function TreeMenu($langs_array, $categories, $level, $ownid) {
		...
		foreach($categories as $catid => $category) {
			if ($catid!=$ownid) {
    			...
    			$digest = $item_digest_handler->getItem($category->getVar('itemid'));
				$langs_array[$catid] = str_repeat('--', $level).$digest->getVar('menu');
				if ($categoriesb = $category_handler->getObjects(new Criteria('parentid', $catid), true)){
